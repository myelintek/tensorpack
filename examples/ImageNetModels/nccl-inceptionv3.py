...
def get_config():
    ...
    BASE_LR = 0.045 * (args.logical_batch / 512.)
    alpha = args.logical_batch / 512.
    ...
    callbacks = [
        ...
        ScheduledHyperParamSetter(
            'learning_rate',
            [(5, 0.03*alpha), (9, 0.01*alpha), (12, 0.006*alpha),
             (17, 0.003*alpha), (22, 1e-3*alpha), (36, 2e-4*alpha),
             (41, 8e-5*alpha), (48, 1e-5*alpha), (53, 2e-6*alpha)]),
...
if __name__ == '__main__':
    ...
    logger.set_logger_dir(os.path.join('train_log', 'inceptionV3-{}'.format(args.load)), action='d')

    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    #NR_GPU = len(args.gpu.split(','))
    NR_GPU = max(get_nr_gpu())
    ...
    trainer = SyncMultiGPUTrainerReplicated(NR_GPU, 1), mode="nccl")
