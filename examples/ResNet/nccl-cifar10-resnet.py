...
class Model(ModelDesc):
    ...
    def optimizer(self):
        ...
        if self._chunk > 1:
            opt = AccumGradOptimizerAlt(opt, self._chunk)
        return opt
...
if __name__ == '__main__':
    ...
    config = TrainConfig(
        model=Model(n=NUM_UNITS, chunk=args.chunk),
        dataflow=dataset_train,
        callbacks=[
            #ModelSaver(),
            InferenceRunner(dataset_test,
                            [ScalarStats('cost'), ClassificationError('wrong_vector')], one_liner=True),
            ScheduledHyperParamSetter('learning_rate',
                                      [(1, 0.1), (8, 0.01), (123, 0.001), (300, 0.0002)])
        ],
        max_epoch=10,
    ...
    launch_train_with_config(config, SyncMultiGPUTrainerReplicated(nr_gpu, mode="nccl"))
